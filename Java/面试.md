...
> 面向切面编程，是一种在无需修改业务代码前提下，对某个业务增加统一的功能，如日志，权限控制，事务等
...
1. Spring 生成 bean 对象时，先实例化的是 target 都西昂
2. 对 target 对象进行属性填充
...
``` shell
java -Xms1024m -Xmx1024m -Xmn384m -XX:+UseG1GC springboot.war
```
...
> **连接：socket 连接，socket 是对 ip + port 抽象**
>
> 占用资源：端口，fd
